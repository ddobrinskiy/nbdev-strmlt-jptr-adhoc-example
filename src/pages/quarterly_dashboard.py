# AUTOGENERATED! DO NOT EDIT! File to edit: ../../notebooks/Quarterly Dashboard.ipynb.

# %% auto 0
__all__ = []

# %% ../../notebooks/Quarterly Dashboard.ipynb 2
from datetime import datetime

import streamlit as st

from streamlit_jupyter import StreamlitPatcher, tqdm

# %% ../../notebooks/Quarterly Dashboard.ipynb 4
st.title("Quarterly Dashboard")

# %% ../../notebooks/Quarterly Dashboard.ipynb 5
st.markdown(
    """

This is a test page demonstrating the use of `streamlit_jupyter`.

If you're seeing this in jupyter, then it's working!

"""
)

# %% ../../notebooks/Quarterly Dashboard.ipynb 7
name = st.text_input("What's your name?", "John")

# %% ../../notebooks/Quarterly Dashboard.ipynb 8
date = st.date_input("Choose a date", datetime.now().date())

# %% ../../notebooks/Quarterly Dashboard.ipynb 9
st.markdown(f"## Hello {name}!\n## The date is {date.strftime('%Y-%m-%d')}")

# %% ../../notebooks/Quarterly Dashboard.ipynb 11
import time

import pandas as pd

st.subheader("A cached dataframe")

@st.cache_data()
def get_data(date):
    for i in tqdm(range(10)):
        time.sleep(0.1)
    return pd.DataFrame(
        {"date": pd.date_range(date, periods=3), "c": [7, 8, 5], "d": [10, 11, 7]}
    ).set_index("date")


df = get_data(date)
st.write(df)

# %% ../../notebooks/Quarterly Dashboard.ipynb 12
st.subheader("Data Editor")
df = pd.DataFrame(
    [
       {"command": "st.selectbox", "rating": 4, "is_widget": True, },
       {"command": "st.balloons", "rating": 5, "is_widget": False, },
       {"command": "st.time_input", "rating": 3, "is_widget": True,},
   ]
)
edited_df = st.experimental_data_editor(df)

# %% ../../notebooks/Quarterly Dashboard.ipynb 13
st.subheader("One more cache example")
@st.cache_resource(ttl=3600)
def get_resource():
    st.write("Getting resource...")
    for i in tqdm(range(5)):
        time.sleep(0.1)
    return {
        "foo": "bar",
        "baz": [1, 2, 3],
        "qux": {"a": 1, "b": 2, "c": 3},
    }


records = get_resource()
st.json(records)

# %% ../../notebooks/Quarterly Dashboard.ipynb 15
st.subheader("Ploting")

import plotly.express as px

df_daily = df.mean(axis="columns").rename("daily_average")
fig = px.line(df_daily, title="Daily mean", width=600)
st.write(fig)

# %% ../../notebooks/Quarterly Dashboard.ipynb 16
st.metric("Speed", 300, 210, delta_color="normal", label_visibility="visible")

# %% ../../notebooks/Quarterly Dashboard.ipynb 17
st.metric("Speed", 300, 210)

# %% ../../notebooks/Quarterly Dashboard.ipynb 18
st.code("print(1+1)", language="python")

# %% ../../notebooks/Quarterly Dashboard.ipynb 19
show_code = st.checkbox("Show code", value=True)

# %% ../../notebooks/Quarterly Dashboard.ipynb 20
if show_code:
    st.code("[i**2 for i in range(100)]")

# %% ../../notebooks/Quarterly Dashboard.ipynb 21
option = st.radio("Choose one option", options=["foo", "bar"], index=1)

# %% ../../notebooks/Quarterly Dashboard.ipynb 22
option = st.selectbox("Selectbox: ", options=["Jane", "Bob", "Alice"], index=0)

# %% ../../notebooks/Quarterly Dashboard.ipynb 23
options = st.multiselect("Multiselect: ", options=["python", "golang", "julia", "rust"])

# %% ../../notebooks/Quarterly Dashboard.ipynb 24
options = st.multiselect(
    "Multiselect with defaults: ",
    options=["nbdev", "streamlit", "jupyter", "fastcore"],
    default=["jupyter", "streamlit"],
)

# %% ../../notebooks/Quarterly Dashboard.ipynb 25
st.subheader("st.text:")
st.text("This is a text")
st.text("This is \n multiline text")
st.code("This is multiline \n code", language=None)
